...
namespace F4SharedMem
{
    public sealed class Writer : IDisposable
    {
        ...
        private static void CreateSharedMemoryArea(string sharedMemeoryAreaFileName, ushort length, out IntPtr hFileMappingObject, out IntPtr lpBaseAddress)
        {
            hFileMappingObject = NativeMethods.CreateFileMapping(new IntPtr(NativeMethods.INVALID_HANDLE_VALUE), IntPtr.Zero, NativeMethods.PageProtection.ReadWrite, 0, length, sharedMemeoryAreaFileName);

            lpBaseAddress = NativeMethods.MapViewOfFile(hFileMappingObject, NativeMethods.SECTION_MAP_READ | NativeMethods.SECTION_MAP_WRITE, 0, 0, IntPtr.Zero);
        }
